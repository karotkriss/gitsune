...
- See the README's "Testing" section for the fixtures-first harness, the full-suite command, and golden-baseline maintenance.
  A bare `flutter test` can silently truncate on memory-constrained machines (it reports `All tests passed!` after only some suites ran, with no error): if a full-suite run's file count looks short, rerun the missing files as explicit arguments (`flutter test test/a_test.dart test/b_test.dart ...`) or in smaller batches rather than trusting one bare invocation.
...
- Syntax highlighting lives in `lib/core/syntax/` (`re_highlight`, E4.4): `language_detection.dart` maps a path to a `re_highlight` language id, `syntax_highlighter.dart`'s `highlightCodeLine`/`highlightDiffLine` are the single per-line entry points both diff decoration and full-file views call, and `gs_syntax_theme.dart` maps `GsTheme`'s five `code*` colors onto both the native `TextSpan` theme and the WebView CSS theme from one scope table so they can't drift.
  `GsSourceView` (`gs_source_view.dart`) is the public full-file widget; it picks the engine from `chooseSyntaxEngine` purely by source size, falling back to `GsSyntaxWebView` above `kNativeSyntaxCharThreshold` for full files only (diffs always stay native).
  The WebView fallback embeds the offline highlight.js bundle vendored at `assets/syntax/` (see its `README.md` for the license and update command) inline into generated HTML, so it never makes a network request.
