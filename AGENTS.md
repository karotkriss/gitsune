...
- `docs/plan/roadmap.md` owns the offline-first repository operating principle; use `lib/core/repository/offline_first_repository.dart` as the seam and `lib/core/repository/current_user_repository.dart` as its single-row exemplar, or `lib/core/repository/todos_repository.dart` as the list exemplar (E3.4 keyset-paginates the whole collection on every `refresh`, then replaces the account's cached rows in one transaction rather than diffing).
- `flutter test`'s default reporter can misattribute lines to whichever test is still running when many files finish concurrently (a long test like `test/architecture_test.dart` dominates the printed output while short files pass silently in the background); the final count and pass/fail result are still correct, but rerun with `--concurrency=1` if you need every file's tests actually printed.
