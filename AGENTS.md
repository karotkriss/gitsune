...
- The v1 build plan lives in `docs/plan/roadmap.md` (phased plan with per-phase exit criteria) and `docs/plan/task-breakdown.md` (pickable tasks per epic, with acceptance notes and dependencies); phase-three development implements against those.
  The roadmap follows the design system that closed phase two (tanuki-orange interactive accent, dark-only v1, liquid glass modest app-wide and heavy on overlays); where `docs/research/design-direction.md` still states the superseded pre-review positions, task E1.6 reconciles it.
