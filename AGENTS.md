...
- The v1 build plan lives in `docs/plan/roadmap.md` (phased plan with per-phase exit criteria) and `docs/plan/task-breakdown.md` (pickable tasks per epic, with acceptance notes and dependencies); phase-three development implements against those.
  The roadmap follows the settled design decisions from the closed design review (tanuki-orange interactive accent, dark-only v1, liquid glass modest app-wide and heavy on overlays), with the design system itself landing via a separate change; where `docs/research/design-direction.md` still states the superseded pre-review positions, task E1.6 reconciles it.
