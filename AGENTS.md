...
- OAuth sign-in (E2.1) lives in `lib/core/auth/`: `GitLabOAuth` delegates the browser leg to `flutter_appauth` (which generates the S256 PKCE pair natively) and performs the code-for-token exchange in Dart so tests can drive it against the fake server; `TokenStore` in `token_store.dart` is the storage seam E2.5 extends with per-account namespacing and refresh.
