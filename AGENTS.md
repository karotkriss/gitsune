...
- Liquid glass: `GlassSurface` (`lib/core/glass/glass_surface.dart`) is the single glass primitive and isolation seam; compose it rather than using `BackdropFilter` directly.
  ...
  The reusable heavy-glass overlays (E1.5) live in `lib/core/glass/glass_overlays.dart` (`GlassModal`, `GlassDrawer`, `GlassBottomSheet`); they ride Material's own modal routes, whose scrim colors are wired once in `buildAppTheme` (dialog barrier, drawer scrim, sheet barrier), so feature overlays compose these instead of styling `showDialog`/`showModalBottomSheet`/`Drawer` by hand.
