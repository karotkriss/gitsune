...
- See the README's "Testing" section for the fixtures-first harness, the full-suite command, and golden-baseline maintenance.
...
- `docs/plan/roadmap.md` owns the offline-first repository operating principle; use `lib/core/repository/offline_first_repository.dart` as the seam and `lib/core/repository/current_user_repository.dart` as its exemplar.
...
- `lib/core/markdown/gs_markdown.dart` (E4.1) is extended, not replaced, by later markdown features: math (`lib/core/markdown/math/`, `flutter_math_fork`) and Mermaid diagrams (`lib/core/markdown/mermaid/`, offline `webview_flutter` + vendored `assets/vendor/mermaid/mermaid.min.js`) each add their own `md.BlockSyntax`/`InlineSyntax` emitting a custom tag, paired with a `MarkdownElementBuilder` keyed to that tag, registered in `GsMarkdown`'s `blockSyntaxes`/`inlineSyntaxes`/`builders`; both degrade to `gsRawSourceFallback` (raw source in a mono/`codeBg` block) rather than throwing.
  Sharp edge: `flutter_markdown_plus` builders keyed by an *existing* built-in tag (`pre`, `code`, `p`, ...) intercept ALL uses of that tag, including the default text-rendering special case for `pre`/`code`, even when the builder returns `null` for elements it doesn't want to handle - this silently blanks ordinary content (it broke plain fenced code blocks when a `pre` builder was added for Mermaid detection). Always parse into a new custom tag via your own `BlockSyntax`/`InlineSyntax` instead of keying a builder to a tag flutter_markdown_plus already understands.
  Vendoring a JS asset for an offline WebView follows the same pattern as `design/`'s vendoring: the file plus its license text side by side, indexed in a README (see `assets/vendor/README.md`) - `assets/vendor/` is a second, app-runtime vendoring location distinct from `design/`'s (design-system-only) one.
