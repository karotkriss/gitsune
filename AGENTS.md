...
- Search (`lib/features/search/`) reads GitLab's `GET /search?scope=projects|issues|merge_requests|blobs&search=<term>`, one network-only repository method pair (`loadFirst*`/`loadNext*`) per scope, each backed by its own `KeysetPaginator`.
  The blobs (code) scope needs the instance's Advanced Search tier: the repository maps GitLab's 4xx scope refusal (except 401/429) to `CodeSearchUnsupportedException`, and the search screen then offers `codeSearchWebUrl` (the instance's web search URL, derived from the client's base URL) instead of an empty section.
