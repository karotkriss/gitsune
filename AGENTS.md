...
- Offline-first data access follows the seam in `lib/core/repository/offline_first_repository.dart`: a `watch()` reactive drift stream the UI subscribes to, and a `refresh()` that fetches over the dio client and writes through to the DB (stream re-emits), swallowing network failures so the DB keeps serving cached data.
  `lib/core/repository/current_user_repository.dart` is the concrete proof against `CurrentUserProfiles`; feature repositories (issues, MRs, todos) implement the same two methods against their own table and endpoint rather than sharing a generic base beyond the interface.
