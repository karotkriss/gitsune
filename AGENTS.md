...
- `docs/plan/roadmap.md` owns the offline-first repository operating principle; use `lib/core/repository/offline_first_repository.dart` as the seam and `lib/core/repository/current_user_repository.dart` as its single-row exemplar, or `lib/core/repository/todos_repository.dart` as the list exemplar (it follows every pagination link on each `refresh`, then replaces the account's cached rows in one transaction rather than diffing).
  The bounded recently-viewed cache (E14.1) is `lib/core/repository/recently_viewed_repository.dart`: `RecentlyViewedCache` holds API-shaped JSON payloads capped at 50 rows per item type per account with least-recently-viewed eviction, `RecentItemRepository<T>` is its per-item seam adapter (feature models round-trip via their `toJson`), and the issue/MR/pipeline detail screens construct one each from `buildAppRouter`'s optional `recentlyViewedCache`.
