...
- `.github/workflows/ci.yml` runs on every PR: format/analyze/`flutter test` (including goldens) on a single pinned ubuntu runner, plus separate Android (`flutter build apk --debug`) and iOS (`flutter build ios --no-codesign`) build-sanity jobs; Flutter version is pinned via `subosito/flutter-action`. Keep goldens on that one OS only to avoid cross-platform pixel drift, and keep this workflow secret-free until release signing (E15.x) is scoped.
