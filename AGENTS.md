...
- `flutter test` is the one command for the whole suite (unit, widget, golden), offline and secret-free; see the README's "Testing" section for the fixtures-first harness (`test/support/fixtures.dart`, `test/support/fake_gitlab_server.dart`, `test/flutter_test_config.dart`).
