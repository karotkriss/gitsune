...
- OAuth sign-in (E2.1) lives in `lib/core/auth/`: `GitLabOAuth` delegates the browser leg to `flutter_appauth` (which generates the S256 PKCE pair natively) and performs the code-for-token exchange in Dart so tests can drive it against the fake server.
  Token persistence (E2.5) is `TokenStore` in `token_store.dart`, namespaced per `AccountKey` in platform secure storage (one JSON value per account, so a rotation persists atomically), and `TokenRefreshCoordinator` in `token_refresh.dart` does lazy, single-flight, single-use-rotation refresh; wire its `readToken`/`refreshToken` into `createGitLabClient`'s seams rather than reading the store directly.
