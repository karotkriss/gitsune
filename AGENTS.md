...
- OAuth sign-in (E2.1) lives in `lib/core/auth/`: `GitLabOAuth` delegates the browser leg to `flutter_appauth` (which generates the S256 PKCE pair natively) and performs the code-for-token exchange in Dart so tests can drive it against the fake server.
  ...
  The sign-in surface (E2.7) is `lib/features/sign_in/sign_in_screen.dart` at route `/signin`: gitlab.com goes straight to `GitLabOAuth`, other hosts are checked via `lib/core/auth/gitlab_instance.dart` (URL parse plus `/api/v4/version` reachability probe), reachable self-hosted hosts open the E2.3 registration wizard by default (`lib/features/sign_in/self_hosted_wizard_screen.dart`, with Application ID validation and named-failure classification in `lib/core/auth/self_hosted_setup.dart`, tested against the Doorkeeper error bodies in `test/fixtures/auth/`) which calls `GitLabOAuth.selfHosted`, and the E2.4 fallback lives in `lib/core/auth/pat_auth.dart` and `lib/features/sign_in/pat_sign_in_screen.dart` (also reachable from the wizard's registration-disabled path).
