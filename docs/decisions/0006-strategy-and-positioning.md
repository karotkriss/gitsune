...
Gitsune's strategy is to compete on breadth and reach rather than on any single narrow surface: cross-platform coverage of both major mobile operating systems, self-hosted GitLab treated as a first-class target rather than a secondary one, a commitment to release under an open-source license, and full feature breadth across merge requests, CI/CD, issues, code browsing, and notifications together.
The exact license remains undecided, as recorded in `README.md`.
